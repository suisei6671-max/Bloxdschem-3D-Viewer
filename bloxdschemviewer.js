        ...
        for (let x = 0; x < 32; x++) {
            for (let y = 0; y < 32; y++) {
                for (let z = 0; z < 32; z++) {
                    const id = arr[i++];
                    if (id === 0) continue;
                    blocks.push({ x: c.x * 32 + x, y: c.y * 32 + y, z: c.z * 32 + z, id: id });
                }
            }
        }
    }
...
async function buildSchem(schem) {
    ...
    for (const [blockId, positions] of blockPositions) {
        ...
        if (!block || !block.textureInfo) continue;
        ...
        for (let face = 0; face < 6; face++) {
            ...
            const tex = await getRotatedTexture(texName, rotation);
    ...
    if (totalPlaced > 0) {
        ...
        updateOrtho(Math.max(max.x - min.x, max.y - min.y, max.z - min.z) * 0.8 + 2);
    }
}
...
document.getElementsByName("angleStyle").forEach(radio => {
    radio.addEventListener("change", (e) => {
        if (e.target.value === "wiki") {
            camera.alpha = Math.PI / 4; camera.beta = Math.atan(1 / Math.sqrt(2));
        } else {
            camera.alpha = Math.PI / 4; camera.beta = Math.atan(Math.SQRT2);
        }
    });
});
...
window.instantCapture = async () => {
    ...
    img.onload = () => {
        ...
        if (found) {
            ...
            finalCanvas.width = cropWidth; finalCanvas.height = cropHeight;
            finalCanvas.getContext("2d").drawImage(tempCanvas, minX, minY, cropWidth, cropHeight, 0, 0, cropWidth, cropHeight);
            const link = document.createElement("a");
            link.download = `capture_${Date.now()}.png`; link.href = finalCanvas.toDataURL("image/png"); link.click();
        }
    };
};
...
document.getElementById("buildBtn").addEventListener("click", async () => {
    ...
    if (input.files.length > 0) {
        await initAssets();
        const schem = await parseSchem(input.files[0]);
        await buildSchem(schem);
    }
});
